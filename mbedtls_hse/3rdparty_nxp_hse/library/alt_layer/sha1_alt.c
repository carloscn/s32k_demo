...
#define SHA_1_OUTPUT_LENGTH			((uint32_t)(0x14UL))/**< SHA-1 output length. */

/* Parameter validation macros. */
#define SHA1_ALT_VALIDATE_RET(cond)         \
    MBEDTLS_INTERNAL_VALIDATE_RET( cond, MBEDTLS_ERR_SHA1_BAD_INPUT_DATA )
#define SHA1_ALT_VALIDATE(cond) 			\
...
==================================================================================================*/
/**
 * 	@brief		Process the input data in ongoing stream context or starts the SHA1 streaming
 ...
 *
 *	@return		#MBEDTLS_ERR_SHA1_BAD_INPUT_DATA for bad input
 *
 *	@return		#MBEDTLS_ERR_SHA1_HW_ACCEL_FAILED for hardware accelator failure
 *
 */
static int Sha1_process( mbedtls_sha1_context *ctx,
...
==================================================================================================*/
/*************************************************************************************************
* Description:  Process the input data in ongoing stream context or starts the SHA1 streaming
* operation if stream operation is not started
************************************************************************************************/
static int Sha1_process( mbedtls_sha1_context *ctx,
                         const unsigned char *data, uint32_t num_bytes )
{
	...
	if(ctx->stream_start_send == FALSE)
	{
		...
		srvResponse = HSE_HashStreamStart(streamId,
				HSE_HASH_ALGO_SHA_1, data, num_bytes, NULL, NULL) ;
...
/*************************************************************************************************
* Description:  This function initializes the specified SHA1 context.
************************************************************************************************/
void mbedtls_sha1_init( mbedtls_sha1_context *ctx )
{
	/* Simple sanity check */
    SHA1_ALT_VALIDATE( ctx != NULL ) ;
...
{
	/* Simple sanity check */
	SHA1_ALT_VALIDATE_RET( ctx != NULL ) ;

	memset(ctx, 0x00, sizeof(mbedtls_sha1_context));
                             ...
                             size_t ilen )
{
	...
	/* Simple sanity check */
    SHA1_ALT_VALIDATE_RET( ctx != NULL );
    SHA1_ALT_VALIDATE_RET( ilen == 0 || input != NULL );
    ...
    if( left && ilen >= fill )
    {
        ...
        if( ( ret = Sha1_process( ctx, ctx->buffer, 64) ) != 0 )
        {
        	return( ret );
        }
    ...
    if( ilen >= 64 )
    {
        ...
        if( ( ret = Sha1_process( ctx, input,  bytes_processed) ) != 0 )
        {
        	return( ret );
        }
                             ...
                             unsigned char output[20] )
{
	...
	hseSrvResponse_t srvResponse = HSE_SRV_RSP_GENERAL_ERROR ;
    uint32_t used;
	uint32_t hashlen = SHA_1_OUTPUT_LENGTH ;
    KeymgmtErrCodeT err;
    uint8_t streamId = INVALID_STREAM_ID;

	/* Simple sanity check */
	SHA1_ALT_VALIDATE_RET( ctx != NULL ) ;
	SHA1_ALT_VALIDATE_RET( (unsigned char *)output != NULL ) ;
	...
	if(ctx->stream_start_send == FALSE)
	{
		...
		*/
		srvResponse = HSE_Hash( HSE_HASH_ALGO_SHA_1, ctx->buffer, used, output, &hashlen ) ;
	}
	...
	else
	{
	    ...
	    srvResponse = HSE_HashStreamFinish(streamId,
	    		HSE_HASH_ALGO_SHA_1, ctx->buffer, used, output, &hashlen ) ;
	}
...
/*************************************************************************************************
* Description:  This function clones the state of a SHA-1 context.
************************************************************************************************/
void mbedtls_sha1_clone( mbedtls_sha1_context *dst,
                         const mbedtls_sha1_context *src )
{
	SHA1_ALT_VALIDATE( dst != NULL );
	SHA1_ALT_VALIDATE( src != NULL );
...
/*************************************************************************************************
* Description:  This function calculates the SHA-1 checksum of a buffer.
************************************************************************************************/
                      ...
                      unsigned char output[20] )
{
	...
	/* Send the request */
    srvResponse = HSE_Hash( HSE_HASH_ALGO_SHA_1, input, ilen, output, &(hashlen) ) ;
...
/*************************************************************************************************
* Description:  SHA-1 process data block .
************************************************************************************************/
                                   ...
                                   const unsigned char data[64] )
{
    ...
    SHA1_ALT_VALIDATE_RET( ctx != NULL );
    SHA1_ALT_VALIDATE_RET( (const unsigned char *)data != NULL );

	/* Process the data block */
	ret = Sha1_process(ctx, data, 64);
...
#endif
/*************************************************************************************************
* Description:  This function releases and clears the specified SHA1 context.
************************************************************************************************/
