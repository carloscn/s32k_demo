...
void mbedtls_md_free( mbedtls_md_context_t *ctx )
{
    ...
    {
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)&&defined(MBEDTLS_HMAC_ALT)
		nxp_hse_hmac_free(ctx->hmac_ctx);
#else
...
int mbedtls_md_setup( mbedtls_md_context_t *ctx, const mbedtls_md_info_t *md_info, int hmac )
{
    ...
    {
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)&&defined(MBEDTLS_HMAC_ALT)
		ctx->hmac_ctx = mbedtls_calloc( 1, sizeof(mbedtls_hmac_context));
		(void)nxp_hse_hmac_init(ctx->hmac_ctx);
		ret = nxp_hse_hmac_setup(ctx->hmac_ctx, ctx);
#else
    ...
    return( ret ); /*if return is error code and not success; then we are exiting the routine*/
}
#undef ALLOC
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)&&defined(MBEDTLS_HMAC_ALT)
	{
		return nxp_hse_hmac_starts_ret(ctx->hmac_ctx, key, keylen);
	}
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)&&defined(MBEDTLS_HMAC_ALT)
	{
		return nxp_hse_hmac_update_ret(ctx->hmac_ctx, input, ilen);
	}
#endif /* MBEDTLS_USE_NXP_HSE_CRYPTO */
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)&&defined(MBEDTLS_HMAC_ALT)
	{
		return nxp_hse_hmac_finish_ret(ctx->hmac_ctx, output);
	}
#endif /* MBEDTLS_USE_NXP_HSE_CRYPTO */
...
int mbedtls_md_hmac_reset( mbedtls_md_context_t *ctx )
{
	...
	if( ( ret = mbedtls_md_starts( ctx ) ) != 0 )
		return( ret );
	/* Reset HMAC Context */
	return nxp_hse_hmac_reset(ctx->hmac_ctx);
#else
    ipad = (unsigned char *) ctx->hmac_ctx;
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)&&defined(MBEDTLS_HMAC_ALT)
	if( (ret = nxp_hse_hmac_ret(ctx.hmac_ctx, key, keylen, input, ilen, output)) != 0)
		goto cleanup;
#else
