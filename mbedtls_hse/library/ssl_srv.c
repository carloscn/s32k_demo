                          ...
                          const mbedtls_ssl_ciphersuite_t * ciphersuite_info )
{
    ...
    for( cur = list; cur != NULL; cur = cur->next )
    {
        ...
        if( ssl->minor_ver < MBEDTLS_SSL_MINOR_VERSION_3 &&
            cur->cert->sig_md != MBEDTLS_MD_SHA512 )   //HERE SHA1 CHANGED TO SHA512
        {
            if( fallback == NULL )
                fallback = cur;
            {
...
read_record_header:
    ...
    {
        mbedtls_md_type_t md_default = MBEDTLS_MD_SHA512;    //MBEDTLS_SHA_1 CHANGED TO MBEDTLS_SHA_512
                                      ...
                                      size_t peer_pmssize )
{
    ...
    return( ret );
#else
    return (nxp_hse_ssl_decrypt_encrypted_pms(ssl, p, end, peer_pms,
                                    ...
                                    size_t pms_offset )
{
    ...
    unsigned char peer_pms[48];
    size_t peer_pmslen;

#if !defined(MBEDTLS_USE_NXP_HSE_CRYPTO)

    unsigned char ver[2];
    unsigned char fake_pms[48];
    unsigned char mask;
    size_t i;
    unsigned int diff;
...
#pragma warning( pop )
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "calculated pms", pms, ssl->handshake->pmslen);
    ...
    /* Get decrypted pms key handle */
    memcpy(pms, peer_pms, sizeof(hseKeyHandle_t));

    MBEDTLS_SSL_DEBUG_BUF(4, "PMS key handle", pms, ssl->handshake->pmslen);
