                             ...
                             void *p_rng )
{
        ...
        ) )
    {
        ...
        if( transform->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1 )
        {
            ...
            MBEDTLS_SSL_DEBUG_BUF( 4, "add_data", add_data, add_data_len );

            mbedtls_md_hmac_update( &transform->md_ctx_enc, add_data,
                                    add_data_len );

            MBEDTLS_SSL_DEBUG_BUF( 4, "Input data", data, rec->data_len );
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "before encrypt: msglen = %d, "
                            ...
                            padlen + 1 ) );
        MBEDTLS_SSL_DEBUG_BUF(4, "raw : data", data, rec->data_len);
        MBEDTLS_SSL_DEBUG_BUF(4, "raw : IV", transform->iv_enc, transform -> ivlen);
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "encrypted : data", data, olen);
        ...
        if( auth_done == 0 )
        {
            ...
            MBEDTLS_SSL_DEBUG_BUF( 4, "MAC'd Input-data", data, rec->data_len );
            ...
            MBEDTLS_SSL_DEBUG_BUF(4, "calculated mac:", mac, transform->maclen);
            rec->data_len += transform->maclen;
            post_avail -= transform->maclen;
            auth_done++;
        }
...
cleanup:
	...
	(void)min_data_len;
	(void)max_data_len;
    return nxp_hse_mbedtls_ssl_cf_hmac(ctx, add_data, add_data_len, data, data_len_secret, output);
#endif
}
                             ...
                             mbedtls_record *rec )
{
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        if( transform->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED )
        {
            ...
            MBEDTLS_SSL_DEBUG_BUF(4, "raw : input data", data, rec->data_len);
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "Iv : used", transform->iv_dec, transform->ivlen);
...
volatile uint32_t test_Delay=0;
...
int mbedtls_ssl_read( mbedtls_ssl_context *ssl, unsigned char *buf, size_t len )
{
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "\n Test -  ssl in offt = %d",ssl->in_offt ) );
...
#ifdef MBEDTLS_USE_NXP_HSE_CRYPTO
    /*
     * Fetching the keyhandle from Cipher context and unloading the key in HSE
     * using the keyhandle
     * */
    nxp_hse_FreeKeys((nxp_hse_cipher_get_keyhandle(&(transform->cipher_ctx_enc))),
    		(nxp_hse_cipher_get_keyhandle(&(transform->cipher_ctx_dec))));

    nxp_hse_cipher_set_keyhandle(&(transform->cipher_ctx_enc));
    nxp_hse_cipher_set_keyhandle(&(transform->cipher_ctx_dec));
...
#ifdef MBEDTLS_USE_NXP_HSE_CRYPTO
    /*
     * Fetching the keyhandle from Message-digest context and unloading the key in HSE
     * using the keyhandle
     * */
    nxp_hse_FreeKeys((nxp_hse_hmac_get_keyhandle(&(transform->md_ctx_enc))),
    		(nxp_hse_hmac_get_keyhandle(&(transform->md_ctx_dec))));

    nxp_hse_hmac_set_keyhandle(&(transform->md_ctx_enc));
    nxp_hse_hmac_set_keyhandle(&(transform->md_ctx_dec));
