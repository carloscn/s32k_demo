...
#else
    return (nxp_hse_tls_prf_generic(md_type, secret, slen, label, random, rlen, dstbuf, dlen ));
#endif
}
...
{
#ifndef MBEDTLS_USE_NXP_HSE_CRYPTO
    int ret = 0;
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    int psa_fallthrough;
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
#else
    (void)tls_prf_get_type;
    return nxp_hse_ssl_populate_transform(transform,ciphersuite, master,
#if defined(MBEDTLS_SSL_SOME_MODES_USE_MAC)
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
    		encrypt_then_mac,
#endif /* MBEDTLS_SSL_ENCRYPT_THEN_MAC */
...
#if defined (MBEDTLS_USE_NXP_HSE_CRYPTO)
    	/* 1st 4 bytes = PMS key_handle
    	 * 2nd 4 bytes = PSK key_handle
    	 * After that copy the Key Exchange type
    	 * */
    	memcpy(handshake->premaster+KEY_EXCHANGE_TYPE_POSITION, &handshake->ciphersuite_info->key_exchange, sizeof(uint8_t));
#endif
...
int mbedtls_ssl_psk_derive_premaster( mbedtls_ssl_context *ssl, mbedtls_key_exchange_type_t key_ex )
{
    ...
    *(p++) = (unsigned char)( psk_len      );
    ...
    return( 0 );
#else
    return (nxp_hse_mbedtls_ssl_psk_derive_premaster(ssl, key_ex ));
#endif
}
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)
    if(mbedtls_mpi_size(&(handshake->ecdh_ctx.d)) <= 4)
    {
    	/*
    	 * fetching the private keyhandle from cipher context and unloading the key from HSE
    	 */
		nxp_hse_FreeKeyhandle((const uint8_t*)handshake->ecdh_ctx.d.p);
		mbedtls_mpi_free(&(handshake->ecdh_ctx.d));
    }
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)
	/*
	 * fetching the premaster keyhandle from SSL context and unloading the key from HSE
	 */
    nxp_hse_FreeKeyhandle((const uint8_t*)handshake->premaster);
#endif
...
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO)
	/*
	 * fetching the master keyhandle from SSl context and unloading the key from HSE
	 */
    nxp_hse_FreeKeyhandle((const uint8_t*)session->master);
	*((hseKeyHandle_t *)(session->master)) = INVALID_KEYHANDLE;
#endif
