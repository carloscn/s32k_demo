                                      ...
                                      int restart_enabled )
{
    ...
    if( mbedtls_mpi_size( &ctx->z ) > blen )
        return( MBEDTLS_ERR_ECP_BAD_INPUT_DATA );
/*
 * As "ecdh_calc_secret_internal" is static function it cannot be override in ALT file.
 * "mbedtls_ecdh_compute_shared" can be override in ALT file which returns shared-secret
 * keyhandle in the ctx->z.
 * Given piece of code will copy the keyhandle to output buffer and its length to olen.
 */
#if defined (MBEDTLS_USE_NXP_HSE_CRYPTO)
    if( mbedtls_mpi_size( &ctx->z ) <= sizeof(hseKeyHandle_t))
    {
    	memcpy(buf, ctx->z.p, sizeof(hseKeyHandle_t));
    	*olen = mbedtls_mpi_size( &ctx->z );
    	return(0);
    }
    return -1;
#else
