                           ...
                           const mbedtls_operation_t operation )
{
	...
	if( ( ctx->cipher_info->flags & MBEDTLS_CIPHER_VARIABLE_KEY_LEN ) == 0 &&
			((ctx->cipher_info->key_bitlen)) != (((uint32_t)key_bitlen) & ~KEYLOADED_FLAG))
		{
			return( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
		}

		ctx->key_bitlen = key_bitlen & ~KEYLOADED_FLAG;
	    ctx->operation = operation;
	    ...
	    {
	    	/* Keyloaded Flag information to lower layer to handle */
                      ...
                      const unsigned char *tag, size_t tag_len )
{
    ...
    if( MBEDTLS_MODE_GCM == ctx->cipher_info->mode )
    {
        if( tag_len > sizeof( check_tag ) )
            return( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
#if defined(MBEDTLS_USE_NXP_HSE_CRYPTO) && defined(MBEDTLS_GCM_ALT)
        /* For Tag verification the firmware expects tag data as input for DECRYPT mode.
         * Firmware returns error HSE_SRV_RSP_VERIFY_FAILED when invalid tag is provided.
         * Tag is verified by Firmware internally.
         */
        (void) memcpy(check_tag, tag, tag_len);
#endif /* defined(MBEDTLS_USE_NXP_HSE_CRYPTO) && defined(MBEDTLS_GCM_ALT) */
